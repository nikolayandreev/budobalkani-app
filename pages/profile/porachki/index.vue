...
export default {
  ...
  beforeCreate() {
    this.$store.dispatch('profile/changeOrders')
  },
  computed: {
    orders() {
  ...
  methods: {
    formatOrderDate(created_at) {
      ...
      return `${date.getDate()}.${month}.${date.getFullYear()}, ${date.getHours()}:${date.getMinutes()}ч.`
    },
...
<style scoped>
.item {
  @apply text-gray-700 shadow-md rounded-md  bg-white mt-8;
}
.item:first-child {
  margin: 0;
}
